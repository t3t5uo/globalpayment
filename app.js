...
app.get("/", (req, res) => {
  ...
  const currency = "USD";
  const sharedSecret = "Y9go4mpkml";
  const responseUrl = "https://www.example.com/responseUrl";
  ...
  const linkUrl = `https://pay.sandbox.realexpayments.com/pay?TIMESTAMP=${timestamp}&MERCHANT_ID=${merchantId}&ACCOUNT=${account}&ORDER_ID=${orderId}&AMOUNT=${amount}&CURRENCY=${currency}&AUTO_SETTLE_FLAG=1&COMMENT1=Mobile%20Channel&HPP_VERSION=2&HPP_CHANNEL=ECOM&HPP_LANG=en&HPP_CAPTURE_ADDRESS=true&HPP_REMOVE_SHIPPING=true&HPP_DO_NOT_RETURN_ADDRESS=false&MERCHANT_RESPONSE_URL=${responseUrl}&CARD_PAYMENT_BUTTON=Pay%20Invoice&CUSTOM_FIELD_NAME=Custom%20Field%20Data&HPP_CUSTOMER_PHONENUMBER_MOBILE=%2B447480947765&SHA1HASH=${sha1Hash}`;

  res.send(`<a href="${linkUrl}">Click To Pay</a>`);
});
